	...
	methods: {
		...
		setupObserverForLogScroll() {
			...
			this.logScrollObserver = new IntersectionObserver(entries => {
				...
				const startLine = this.visibleLines[0] || 1;

				this.log(() => `logScrollObserver intersecting / new startLine: ${startLine}`);
				const scrollRatio = (startLine - 1) / (this.totalLines - this.visibleLines.length);
