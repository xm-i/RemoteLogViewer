...
public class TextFileViewerModel : ModelBase<TextFileViewerModel> {
	...
	/// </summary>
	public async Task Grep(string? query, long startLine, bool ignoreCase, bool useRegex, CancellationToken ct) {
		if (this._sshService is null) {
			throw new InvalidOperationException();
		}
		...
		await foreach (var lines in this.GrepOperation.RunAsync(this._sshService, this.OpenedFilePath.Value, query, this.FileEncoding.Value, startOffset, startLine, max, ignoreCase, useRegex, ct).ChunkForAddRange(TimeSpan.FromMilliseconds(500), null, ct)) {
