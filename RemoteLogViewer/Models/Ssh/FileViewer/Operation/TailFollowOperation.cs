...
public sealed class TailFollowOperation : ModelBase<TailFollowOperation>, ITailFollowOperation {
	...
	public TailFollowOperation(IOperationRegistry operations, IByteOffsetIndex byteOffsetIndex, ILogger<TailFollowOperation> logger) : base(logger) {
		this._operations = operations;
		this._byteOffsetIndex = byteOffsetIndex;
	}

	public async IAsyncEnumerable<long> RunAsync(ISshService sshService, string? filePath, string? encoding, long currentLastLine, int chunkSize, [EnumeratorCancellation] CancellationToken ct) {
